...
public function index($conversationId = false, $year = false, $month = false)
{
	...
	else if ($this->responseType === RESPONSE_TYPE_VIEW) $startFrom = min($startFrom, $conversation["countPosts"] - 1);

	//die($startFrom." o o ". $conversation["countPosts"]);
...
/////////////////////////////////////////////////////////////////////////
	...
	else if ($this->responseType === RESPONSE_TYPE_VIEW) {
...
//$no =substr($conversationId, strpos($conversationId, "-") + 1);	
//echo "<br /> ss ". $no." ss ";
...
if ($slug and (strpos($conversationId, "-") === false or substr($conversationId, strpos($conversationId, "-") + 1) != $slug)) {
		...
		$medicare="";
		$medicare = "?medicare=".R('medicare');
		
		
		// Set up the reply form.
		$replyForm = ETFactory::make("form");
		$replyForm->action = URL("conversation/reply/".$conversation["conversationId"]).$medicare;
		$replyForm->setValue("content", $conversation["draft"]);
		$this->data("replyForm", $replyForm);
		$this->data("replyControls", $this->getEditControls("reply"));
		...
		$this->data("medicare", R('medicare'));


		$this->render("conversation/medicare", $data);
...
public function reply($conversationId = false)
{
	...
	if (!$conversation["canReply"]) {
		$this->renderMessage(T("Error"), T("message.noPermission"." system "));
		return;
	}
	...
	// Add a reply.
		...
		if ($model->errorCount())
		  ...
		  if ($this->responseType === RESPONSE_TYPE_AJAX) {
			...
			elseif($medicare=R('medicare')){
		
			$medicare=preg_replace("/\-/","/",$medicare);
			$this->redirect($medicare);
		
		}
			elseif ($this->responseType === RESPONSE_TYPE_DEFAULT) {
				
				
				//$this->redirect('http://localhost/medicare/index.php/feeds/myfeeds/');
...
public function deletePost($postId = false)
{
	...
	else if ($this->responseType === RESPONSE_TYPE_DEFAULT) {
		redirect(URL(R("return", postURL($postId))));
	}
...
protected function formatPostForTemplate($post, $conversation)
{
	...
	if ($this->responseType === RESPONSE_TYPE_VIEW) {
			...
			$formatted["info"][] = "<a href='#' class='time' title='".date(T("date.full"), 
	$post["time"])."'>".(!empty($conversation["searching"]) ? T("Context") : $date)."</a>";

		}
	else {
		$formatted["info"][] = "<a href='".URL(postURL($post["postId"]))."' class='time' title='".date(T("date.full"), 
	$post["time"])."'>".(!empty($conversation["searching"]) ? T("Context") : $date)."</a>";
	  }
	...
	if (!$post["deleteMemberId"]) {
		...
		if ($conversation["canReply"]||R('medicare'))
			
			if ($this->responseType === RESPONSE_TYPE_VIEW) {
			
			
		 $formatted["controls"][] = "<a href='http://localhost/medicare/index.php/feeds/myfeeds/".$conversation["conversationId"]."?quote=".$post["postId"]."#reply' title='".T("Quote")."' class='control-quote'>".T("Quote")."</a>";
		...
		if ($canEdit||R('medicare')) {
			
			if ($this->responseType === RESPONSE_TYPE_VIEW) {
			
			$formatted["controls"][] = "<a href='http://localhost/medicare/esotalk/conversation/deletePost/".$post["postId"]."?token=".ET::$session->token."&medicare=".R('medicare')."' title='".T("Delete")."' class='control-delete'>".T("Delete")."</a>";
		...
		if ($canEdit||R('medicare'))
			{
			 ...
			 if ($this->responseType === RESPONSE_TYPE_VIEW){
				 ...
				 $current_url=R('medicare');
				 
				 $formatted["controls"][] = "<a href='".URL("conversation/restorePost/".$post["postId"]."?token=".ET::$session->token)."&medicare=".$current_url."' title='".T("Restore")."' class='control-restore'>".T("Restore")."</a>";
				 }
			 
			else 	$formatted["controls"][] = "<a href='".URL("conversation/restorePost/".$post["postId"]."?token=".ET::$session->token)."' title='".T("Restore")."' class='control-restore'>".T("Restore")."</a>";
