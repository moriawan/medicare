...
public function remote_login()
{
	...
	if (true) {
		...
		if (ET::$session->login( $username , $password , 1)){
			
			$uid = ET::$session->userId ;
			...
			$this->redirect($medicare.'index.php/login/redirect/'.$uid."/".$token.'/'.$redirectURL);
...
public function curl_login()
{
	...
	//die($username." and ".$password." ret = ".$medicare);
	...
	if (true) {
		...
		if (ET::$session->login( $username , $password , 1)){
			...
			//die($medicare."index.php/login");
			
			$this->redirect($medicare."?ret=".$uid.$token);
