...
// Title ?>
<h1 id='conversationTitle' style="border-bottom:none"><?php
...
// Channel ?>
<a href='#' class='channel channel-<?php echo $conversation["channelId"]; ?>' title='<?php echo sanitizeHTML($conversation["channelDescription"]); ?>'><?php echo sanitizeHTML($conversation["channelTitle"]); ?></a>
<?php

// Star
echo star($conversation["conversationId"], $conversation["starred"],1)."\n";
...
<li class='scrubber-op<?php if ($data["startFrom"] == 0 and empty($data["year"])): ?> selected<?php endif; ?>' 
data-index='first'><a href='<?php echo $medicare."/p1"; ?>'><?php echo T("Original Post"); ?></a></li>
<?php
...
<li class='scrubber-now' data-index='last'<?php if ($latestYear != $currentYear or $latestMonth != $currentMonth): ?> style='display:none'<?php endif; ?>><a href='<?php //echo $medicare."/last";
echo "#"; ?>'><?php echo T("Now"); ?></a></li>
</ul>
...
$ceiling_data=ceil($data["startFrom"] / C("esoTalk.conversation.postsPerPage") + 1)-1;


if ($data["startFrom"] > 0): ?>
<li class='scrubberMore scrubberPrevious'><a href='<?php echo $medicare."/p".$ceiling_data; ?>'>&lsaquo; <?php echo T("Older"); ?></a></li>
<?php endif; ?>
...
$floor_data=$data["startFrom"] / C("esoTalk.conversation.postsPerPage") + 2;
$floor_data=(int)$floor_data;

//echo " a ".$floor_data."<br />";
		
//die($data["startFrom"]." ass ".makeURL("p".$floor_data));

if ($data["startFrom"] + C("esoTalk.conversation.postsPerPage") < $conversation["countPosts"]): ?>
<li class='scrubberMore scrubberNext'><a href='<?php echo $medicare."/p".$floor_data; ?>'><?php echo T("Newer"); ?> &rsaquo;</a></li>
<?php endif; ?>
...
if (!$conversation["canReply"]&&!$medicare) {
