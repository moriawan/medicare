...
function R($key, $default = "")
{
	...
	if(preg_match("/\?quote=/",$page_url)&&$key=='quote')
		{
			//$ret=preg_replace("/[^=]*=/","",$page_url);
			
			
			$ret=preg_replace("/&.+/","",$page_url);
			$ret=preg_replace("/[^=]*=/","",$ret);
			
			//die($ret." HELLO ".$page_url);
			
			
			return $ret;
		}
	else if(preg_match("/medicare=/",$page_url)&&$key=='medicare')
		{
	...
	//die(preg_match("/q/","quote",$matches));
	//die($key);
	//die($_GET[$key]);
