...
public function getActivity($member, $offset = 0, $limit = 11)
{
	...
	//http://localhost/medicare/esotalk/conversation/post/31
...
public function getNotifications($limit = 5)
{
	...
	return null;
		...
		//echo "<br />destroyer cunt ".ET::$session->userId." <br /><br />";
		//ET::$session->userId=2;
	}
	...
	/*	foreach($item["data"] as $k => $v) 
			echo $item["type"]." ".$item["read"]." ".$k." ".$v." s ".$i."<br />";
		echo "<br />";
		*/
...
list($item["body"], $item["link"]) = call_user_func_array(self::$types[$item["type"]][self::PROJECTION_NOTIFICATION], array(&$item))+array(null, null);
	...
	//parse for medicare - codeignitor
	
		
		switch($item["type"]){
			
			case "appointment": $item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/1");
					break;
			case "eAppointment": $item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/2");
					break;
			case "subscribe": $item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/3");
					break;
			case "prescription": $item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/4");
					break;
			case "permissionRequest": $item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/5");
					break;
			case "permissionGrant": $item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/6");
					break;
			case "post": $item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/0");
					break;
			
			
			}
		
		//$item["link"]=preg_replace("/esotalk\/conversation\/post/","index.php/post/index",$item["link"]."/".$item["TId"]);


		$notifications[] = $item;
	}

	//die($i." xx". $notifications[0]['type']);
...
// handle medicare notifications


public static function appointmentAddNotification($item)
{
		$protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
		$urx = $protocol . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		
		
		$urx = preg_replace("/esotalk.*/","",$urx);
		
		//die($urx." ss");
		
		return array(
		sprintf(T("%s booked an appointment with you"),  "<strong>".$item["fromMemberName"]."</strong>"),
		$urx."index.php/post/index/".$item['TId']
	);
	
}


public static function eEAppointmentAddNotification($item)
{
		$protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
		$urx = $protocol . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		...
		return array(
		sprintf(T("%s booked an e-appointment with you"),  "<strong>".$item["fromMemberName"]."</strong>"),
		$urx."index.php/post/index/".$item['TId']
	);
...
public static function subscriberAddNotification($item)
{
		$protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
		$urx = $protocol . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		...
		return array(
		sprintf(T("%s subscribed to your feeds"),  "<strong>".$item["fromMemberName"]."</strong>"),
		$urx."index.php/post/index/".$item['TId']
	);
...
public static function prescriptionProcessNotification($item)
{
		$protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
		$urx = $protocol . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		...
		return array(
		sprintf(T("%s wrote a prescription for you"),  "<strong>".$item["fromMemberName"]."</strong>"),
		$urx."index.php/post/index/".$item['TId']
	);
...
public static function permissionRequestNotification($item)
{
		$protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
		$urx = $protocol . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		...
		return array(
		sprintf(T("%s requested permission to view your profile"),  "<strong>".$item["fromMemberName"]."</strong>"),
		$urx."index.php/post/index/".$item['TId']
	);
...
public static function permissionGrantNotification($item)
{
		$protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
		$urx = $protocol . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		...
		return array(
		sprintf(T("%s granted you permission to view his\her profile"),  "<strong>".$item["fromMemberName"]."</strong>"),
		$urx."index.php/post/index/".$item['TId']
	);
...
ETActivityModel::addType("appointment", array(
	"notification" => array("ETActivityModel", "appointmentAddNotification")
	
));

ETActivityModel::addType("eAppointment", array(
	"notification" => array("ETActivityModel", "eEAppointmentAddNotification")
	
));

ETActivityModel::addType("subscribe", array(
	"notification" => array("ETActivityModel", "subscriberAddNotification")
	
));

ETActivityModel::addType("prescription", array(
	"notification" => array("ETActivityModel", "prescriptionProcessNotification")
	
));

ETActivityModel::addType("permissionRequest", array(
	"notification" => array("ETActivityModel", "permissionRequestNotification")
	
));

ETActivityModel::addType("permissionGrant", array(
	"notification" => array("ETActivityModel", "permissionGrantNotification")
